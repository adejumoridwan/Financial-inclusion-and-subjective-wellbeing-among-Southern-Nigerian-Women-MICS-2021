...
model <- '
  #measurement model
    ...
    usage =~ FN2 + FN4A + FN4C + FN4D + FN4E + FN4F + FN4G + FN4H + FN4X + FN4NR + FN5 + insurance
    FI =~ awareness + usage
    SEF =~ welevel + zone + WAGE + HH6 + MSTATUS
    SW =~ LS1 + LS2 + LS3 + LS4
'
# cfa model
fit <- cfa(model, data = wm,sampling.weights="wmweight", ordered=T)
...
model <- '
  # measurement model
    awareness =~ FN1A + FN1B + FN1C + FN1D + FN1E + FN1F + FN1G + FN1H + FN1I + FN1J + FN1K + FN1L
    usage =~ FN2 + FN4C + FN4D + FN4E + FN4NR + insurance
    FI =~ awareness + usage
    SW =~ LS1 + LS3 + LS4
  # regression
    SW ~ c*FI
'
# sem model
fit <- sem(model, data = wm,sampling.weights="wmweight", ordered=T)
